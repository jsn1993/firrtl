  ...
  val options = Seq(
    new ShellOption[String](
      ...
      toAnnotationSeq = (a: String) => a match {
        ...
        case "pymtl3"               => Seq(RunFirrtlTransformAnnotation(new PyMTL3Emitter),
                                           EmitCircuitAnnotation(classOf[PyMTL3Emitter]))
      ...
      shortOption = Some("E"),
      helpValueName = Some("<chirrtl|high|middle|low|verilog|mverilog|sverilog|pymtl3>") ) )
  ...
  val options = Seq(
    new ShellOption[String](
      ...
      toAnnotationSeq = (a: String) => a match {
        ...
        case "pymtl3"               => Seq(RunFirrtlTransformAnnotation(new PyMTL3Emitter),
                                           EmitAllModulesAnnotation(classOf[PyMTL3Emitter]))
        case _                      => throw new PhaseException(s"Unknown emitter '$a'! (Did you misspell it?)") },
...
final case class EmittedPyMTL3Module(name: String, value: String, outputSuffix: String) extends EmittedModule
final case class EmittedPyMTL3Circuit(name: String, value: String, outputSuffix: String) extends EmittedCircuit

case class EmittedPyMTL3CircuitAnnotation(value: EmittedPyMTL3Circuit)
    extends EmittedCircuitAnnotation[EmittedPyMTL3Circuit]
case class EmittedPyMTL3ModuleAnnotation(value: EmittedPyMTL3Module)
    extends EmittedModuleAnnotation[EmittedPyMTL3Module]

class PyMTL3Emitter extends SeqTransform with Emitter {
  def inputForm = LowForm
  def outputForm = LowForm
  val outputSuffix = ".py"
  val tab = "  "
  ...
  /** Turn Params into Python Strings */
  def stringify(param: Param): String = param match {
    case IntParam(name, value) =>
      val lit =
        if (value.isValidInt) {
  ...
  def getRenderer(m: Module, moduleMap: Map[String, DefModule])(implicit writer: Writer): PyMTL3Render = {
    new PyMTL3Render(m, moduleMap)(writer)
  }

  def getRenderer(descriptions: Seq[DescriptionAnnotation],
                  m: Module,
                  moduleMap: Map[String, DefModule])(implicit writer: Writer): PyMTL3Render = {
    val newMod = new AddDescriptionNodes().executeModule(m, descriptions)

    newMod match {
      case DescribedMod(d, pds, m: Module) => new PyMTL3Render(d, pds, m, moduleMap)(writer)
      case m: Module => new PyMTL3Render(m, moduleMap)(writer)
    }
  }

  class PyMTL3Render( description: Description,
                      portDescriptions: Map[String, Description],
                      m: Module,
                      moduleMap: Map[String, DefModule])(implicit writer: Writer) {
    ...
    // turn strings into Seq[String] pymtl3 comments
    def build_comment(desc: String): Seq[Seq[String]] = {
      val lines = desc.split("\n").toSeq
    ...
    def emitPyMTL3Bind(overrideName: String, body: String): DefModule = {
      build_netlist(m.body)
      build_ports()
  ...
  def emit(state: CircuitState, writer: Writer): Unit = {
    ...
    circuit.modules.foreach {
      case dm @ DescribedMod(d, pds, m: Module) =>
        val renderer = new PyMTL3Render(d, pds, m, moduleMap)(writer)
        renderer.emit_pymtl3()
      case m: Module =>
        val renderer = new PyMTL3Render(m, moduleMap)(writer)
        renderer.emit_pymtl3()
      case _ => // do nothing
    }
  }
  ...
  override def execute(state: CircuitState): CircuitState = {
    val newAnnos = state.annotations.flatMap {
      case EmitCircuitAnnotation(_) =>
        val writer = new java.io.StringWriter
        emit(state, writer)
        Seq(EmittedPyMTL3CircuitAnnotation(EmittedPyMTL3Circuit(state.circuit.main, writer.toString, outputSuffix)))
      ...
      case EmitAllModulesAnnotation(_) =>
        ...
        circuit.modules flatMap {
          case dm @ DescribedMod(d, pds, module: Module) =>
            val writer = new java.io.StringWriter
            val renderer = new PyMTL3Render(d, pds, module, moduleMap)(writer)
            renderer.emit_pymtl3()
            Some(EmittedPyMTL3ModuleAnnotation(EmittedPyMTL3Module(module.name, writer.toString, outputSuffix)))
          case module: Module =>
            val writer = new java.io.StringWriter
            val renderer = new PyMTL3Render(module, moduleMap)(writer)
            renderer.emit_pymtl3()
            Some(EmittedPyMTL3ModuleAnnotation(EmittedPyMTL3Module(module.name, writer.toString, outputSuffix)))
          case _ => None
        }
